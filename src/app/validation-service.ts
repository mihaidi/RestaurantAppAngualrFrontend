export class ValidationService {
    static getValidatorErrorMessage(validatorName: string, validatorValue?: any) {
        let config = {
            ...
            'invalidFirstName': 'Invalid First Name. First name must contains only letters',
            'invalidLastName': 'Invalid Last Name. Last name must contains only letters',
    ...
    static firstNameValidation(control) {
    ...
    static lastNameValidation(control) {
        if (control.value && control.value.match(/^[a-zA-Z]+$/)) {
            return null;
        } else {
